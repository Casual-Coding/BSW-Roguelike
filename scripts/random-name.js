BSWG.randomName = new function() {
    ...
    var generate = function(minLen, maxLen) {
        ...
        var ret = addVowels(randList(prefix));
        var n = ~~(Math.pow(Math.random(), 2.0) * 3) + 0;
        for (var i=0; i<n; i++) {
            for (var k=0; k<100; k++) {
                var t = addVowels(randList(middle));
                if (endsVowel(ret) !== startsVowel(t)) {
                    ret += t;
                    break;
                }
            }
