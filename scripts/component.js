...
BSWG.component_SawBlade = {
	...
	init: function(args) {
		...
		this.size      = args.size || 1;
		this.nteeth    = [12, 14, 16][this.size-1];
		...
		for (var i=0; i<this.nteeth; i++) {
			...
			wheelVerts[i] = new b2Vec2(
				Math.cos(a) * this.size * 0.2, Math.sin(a) * this.size * 0.2
			);
		}
		...
		for (var i=0; i<this.nteeth; i++) {
			...
			var ac = (i+0.5)/this.nteeth * Math.PI * 2.0;
			var j = (i+1) % this.nteeth;
			tverts[0] = new b2Vec2(wheelVerts[i].x, wheelVerts[i].y);
			tverts[1] = new b2Vec2(Math.cos(ac) * (this.size+this.toothSize) * 0.2, Math.sin(ac) * (this.size+this.toothSize) * 0.2);
		...
		cjp.motorType = (false ? 1 : 2) * 10 + this.size;
		this.jpoints = [ cjp ];
		...
		for (var i=0; i<this.obj.verts.length; i++) {
			...
			this.meshObjs[i] = BSWG.generateBlockPolyMesh(tmpObj, 0.7, center, null, i === 0 ? 0.1 : 0.05);
